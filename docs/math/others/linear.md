...
LU分解是最为简单的一种分解，对于一个可逆的$n\times n$方阵$A$，我们希望找到两个方阵$L,U$，使得
$$
A=LU
$$

其中$U$是上三角的，$L$是下三角的，并且对角线元素为$1$，显然，如果它存在，则我们可以使用标准的高斯消元法求出$U$，此时的$L$中的元素就等于每次消元时乘的系数（这个很容易证明）。然而，遗憾的是，并非所有可逆矩阵都可以进行LU分解，这是因为如果我们只考虑它的$k$阶顺序主子式，那么它在前$k$轮消元时可以认为是单独的被分解的矩阵，此时需要它也可逆。因此，我们得到结论：

> $n$阶方阵可被LU分解，当且仅当它的所有阶顺序主子式都不为$0$

于是，对于一般的可逆矩阵，我们可以让它乘上一个行置换矩阵$P$，得到
$$
PA=LU\Rightarrow A=PLU
$$

这种分解方式称为PLU分解，它对于所有可逆矩阵都存在。LU分解的主要作用在于它可以在利用高斯消元法的速度的同时复用高斯消元的结果

---

现在，假定$A$是实对称的正定矩阵，那么它可以进行$LU$分解，但是此时更强的结论是Cholesky分解，即存在下三角矩阵$L$，使得
$$
A=LL^{T}
$$

在$L$的对角线元素都为正的情况下这种分解是唯一的。这个结论可以依靠数学归纳法证明

### QR分解

对于$m\times n$的矩阵$A$，如果$m\ge n$，我们总可以将它写为
$$
A=QR
$$

其中$Q$是$m\times m$的正交矩阵，$R$是$m\times n$的上三角矩阵。另一种形式是$Q$为$m\times n$的列正交矩阵（即满足$Q^{T}Q=I$），二$R$是$n\times n$的上三角矩阵。这样的分解方式称为QR分解，很容易通过施密特正交化方法验证这类分解的存在性。

QR分解的一个最经典用途在于最小二乘求解
$$
\\|Ax-b\\|^{2}
$$

将$A$分解为$m\times m$的正交矩阵$Q$和$m\times n$的上三角矩阵$R$，则
$$
\\|Ax-b\\|^{2}=\\|Rx-Q^{T}b\\|^{2}
$$
...
R_{1}\\\\ 0
\end{pmatrix},Q_{1}\in\mathbb{R}^{m\times n},Q_{2}\in\mathbb{R}^{m\times(m-n)},R_{1}\in\mathbb{R}^{n\times n}
$$

这种写法实际上揭示了两种QR分解的一般关系。考虑
$$
Q^{T}b=:\begin{pmatrix}
c_{1}\\\\ c_{2}
\end{pmatrix},c_{1}\in\mathbb{R}^{n},c_{2}\in\mathbb{R}^{m-n}
$$

于是有
$$
\\|Ax-b\\|^{2}=\\|R_{1}x-c_{1}\\|^{2}+\\|c_{2}\\|^{2}
$$

于是最小二乘解即为线性方程$R_{1}x-c_{1}=0$的解，残差即为$\\|c_{2}\\|^{2}$.这种最小二乘求解方式理论上来讲能够获得极高的性能，但是实际中并不是最推荐的方法（通常是采用SVD分解），因为其在不满秩的情况下会出现无法收敛到最小范数解的情况

QR分解的另一个用途是数值计算特征值。对于方阵$A$，假定
$$
A_{0}=A,A_{k}=Q_{k}R_{k},A_{k+1}=R_{k}Q_{k}
$$

那么$A_{k}$最终将趋近于一个上三角的矩阵，其对角线元素就是特征值
